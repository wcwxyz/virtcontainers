...
func (n *cnm) add(config *NetworkConfig) ([]NetworkInterfacePair, error) {
	return []NetworkInterfacePair{}, nil
}

// join switches the current process to the specified network namespace
// for the CNM network.
func (n *cnm) join(config NetworkConfig) error {
	return nil
}

// remove unbridges and deletes TAP interfaces. It also removes virtual network
// interfaces and deletes the network namespace for the CNM network.
func (n *cnm) remove(config NetworkConfig, netPairs []NetworkInterfacePair) error {
	return nil
}
